...
class RemoAPI:
            ...
            for actor in actors:
                if "role_name" in actor.attributes.keys():
                    if actor.attributes["role_name"] == "hero":
                        print("Got ego id " + str(actor.id))
                        self.active_ego_id = int(actor.id)
    ...
    def replace_ego_vehicle(self):
        ...
        self.active_ego = actor.id
        print("New ego id is " + str(actor.id))
    ...
    def enable_environment_object(self, id):
        self.world.enable_environment_objects([id])
